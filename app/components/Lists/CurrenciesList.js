...
const CurrenciesList = () => {
  const {currencyModal, currency, currencies} = useSelector(state => state);
  const dispatch = useDispatch();

  const handleClick = c => {
    // if (c.id !== country.id) {
    dispatch(chooseCurrency(c));
    // }
  };

  const hide = () => dispatch(hideCurrencyModal());
      ...
      <Animated
        ...
        <TouchableOpacity
          ...
          <ImageLoaderContainer
            img={item.country.thumb}
            style={styles.countryFlag}
            resizeMode={isIOS ? 'cover' : 'stretch'}
          />
          <Text style={styles.phoneNo}>{item.currency_symbol}</Text>
        </TouchableOpacity>
      </Animated>
    );
  };
